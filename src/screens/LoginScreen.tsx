...
export default function LoginScreen() {
  ...
  return (
    ...
    >
      ...
      {sent ? (
        <div className="rounded-card bg-card px-[18px] py-6 text-center text-[13.5px] leading-[1.8] text-ink3 shadow-card">
          <b className="mb-1.5 block font-round text-[15.5px] font-medium text-ink2">
            메일을 보냈습니다
          </b>
      ...
      {!invited && (
        <div className="mt-3.5 flex flex-col items-center gap-1">
          {mode === 'in' && (
            <>
              <button type="button" onClick={() => go('up')} className="py-2 text-[13.5px] text-ink3">
                계정이 없으신가요? 새로 만들기
              </button>
              <button
                type="button"
                onClick={() => go('forgot')}
                className="py-1 text-[13px] text-ink3"
              >
                비밀번호를 잊으셨나요?
              </button>
            </>
          ...
          {mode !== 'in' && (
            <button type="button" onClick={() => go('in')} className="py-2 text-[13.5px] text-ink3">
              로그인으로 돌아가기
            </button>
          )}
        </div>
      ...
      <p className="mb-auto mt-6 px-1 text-center text-[12px] leading-[1.7] text-ink3">
        {invited
