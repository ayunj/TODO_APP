...
export default function TaskRow({ task, showDate = false }: { task: Task; showDate?: boolean }) {
  ...
  return (
    ...
    >
      ...
      >
        <span
          className={`block break-words text-[15.5px] font-medium leading-[1.45] ${
            task.done ? 'text-done line-through decoration-1' : ''
          }`}
        >
          {task.title}
          {task.priority === 3 && <span className="ml-1.5 text-[13.5px] text-star">★</span>}
        </span>

        {task.memo && (
          <span
            className={`mt-2 block whitespace-pre-wrap break-words rounded-xl text-[13px] leading-[1.55] ${
        ...
        <span className="mt-1.5 flex flex-wrap items-center gap-2 text-[12px] text-ink3">
          ...
          {task.done && task.doneBy && (
            <span className="rounded-full bg-accent-soft px-[9px] py-0.5 text-[11px] font-medium text-accent">
              {task.doneBy}
            </span>
          )}
        </span>
      ...
      <span className="flex flex-none flex-col items-center gap-0.5">
        ...
        <button
          ...
          className="grid h-7 w-7 place-items-center rounded-[10px] text-[16.5px] text-faint active:bg-sunk active:text-high"
        >
          ×
        </button>
      </span>
