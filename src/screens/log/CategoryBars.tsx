...
export default function CategoryBars({ monthTasks }: { monthTasks: Task[] }) {
  ...
  return (
    <div className="mt-2.5 rounded-card bg-card px-[15px] py-4 shadow-card">
      <div className="mb-3 text-[13.5px] font-medium text-ink2">카테고리별 완료</div>
      {rows.map((r) => (
        <div
          key={r.c.id}
          className="grid grid-cols-[78px_1fr_34px] items-center gap-2.5 py-[7px]"
        >
          <span className="flex items-center gap-[7px] overflow-hidden text-ellipsis whitespace-nowrap text-[13.5px] text-ink2">
          ...
          <span className="text-right font-mono text-[12.5px] text-ink3">
            {r.done}/{r.total}
          </span>
        </div>
      ))}
