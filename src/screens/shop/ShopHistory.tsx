  ...
  return (
    <details className="mt-6">
      ...
      <div className="mt-1.5">
        {days.map(([day, list]) => (
          <div key={day} className="mb-3.5">
            <div className="mb-1.5 flex items-baseline justify-between gap-2">
              <span className="text-[12px] font-medium text-ink2">{label(day)}</span>
              <span className="font-mono text-[11px] text-ink3">{list.length}</span>
            </div>
              ...
              {list.map((i) => (
                <li key={i.id}>
                  <button
                    ...
                    title="다시 담기"
                    className="rounded-full bg-card px-3 py-1.5 text-[12.5px] text-ink2 shadow-card active:scale-95"
                  >
                    {i.title}
                  </button>
                </li>
        ...
        <p className="pb-1 text-[11.5px] leading-[1.6] text-ink3">
