    ...
    <>
      <div className="mb-[14px] flex gap-[9px]">
        {[
          ...
          <div key={s.k} className="flex-1 rounded-2xl bg-card px-[13px] py-3 shadow-card">
            <span className="block text-[11px] text-ink3">{s.k}</span>
            <span className="mt-0.5 block font-round text-[20.5px] font-medium">
              <span className="font-mono">{s.v}</span>
              {s.unit && <small className="text-[12.5px] text-ink3">{s.unit}</small>}
            </span>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-[3px] rounded-card bg-card px-2 py-3 shadow-card">
        {DOW.map((w, i) => (
          <span
            key={w}
            className={`pb-[7px] text-center text-[11.5px] ${i === 0 ? 'text-high' : 'text-ink3'}`}
          >
            {w}
          </span>
        ))}
