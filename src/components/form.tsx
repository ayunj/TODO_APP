...
export function Field({
  ...
  return (
    <div className="mb-4">
      <label htmlFor={htmlFor} className="mb-2 block text-[13px] font-medium text-ink2">
        {label}
      </label>
      {children}
    </div>
...
export function Hint({ children }: { children: React.ReactNode }) {
  return <div className="-mt-1.5 mb-4 text-[12px] leading-[1.6] text-ink3">{children}</div>;
}
...
export function GoButton(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      type="button"
      {...props}
      className="mt-1 w-full rounded-2xl bg-accent p-4 text-[15.5px] font-medium text-white active:scale-[.99]"
    />
  );
}
...
export function DangerButton(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      type="button"
      {...props}
      className="mt-[9px] w-full rounded-2xl bg-danger-soft p-[15px] text-[14.5px] font-medium text-high"
    />
  );
}
      ...
      {categories.map((c) => {
        ...
        return (
          <button
            ...
            className={`inline-flex items-center gap-1.5 rounded-full px-[15px] py-2.5 text-[13.5px] shadow-card ${
    ...
    <div className="flex gap-1">
      {([1, 2, 3] as Priority[]).map((p) => (
        <button
          ...
          onClick={() => onChange(p)}
          className={`p-0.5 text-[26.5px] leading-none ${p <= value ? 'text-star' : 'text-edge'}`}
        >
          ★
        </button>
      ))}
    ...
    <div className="flex flex-wrap gap-[7px]">
      {PALETTE.map((c) => (
        <button
          ...
          onClick={() => onChange(c)}
          className="relative grid h-9 w-9 place-items-center rounded-full text-[15.5px] font-bold text-white"
          style={{ background: c }}
        >
          {c === value ? '✓' : ''}
        </button>
