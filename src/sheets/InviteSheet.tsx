...
/** 코드가 든 문구 한 덩이를 복사하거나 그대로 보낸다. */
export default function InviteSheet({ id }: { id: string }) {
  const { rooms } = useRooms();
  const { openSheet, closeSheet } = useUi();

  const room = rooms.find((r) => r.id === id) ?? null;
  const back = () => openSheet({ kind: 'room', id });

  if (!room) {
    return (
      <Sheet title="초대하기" onClose={closeSheet} onBack={back}>
  ...
  return (
    <Sheet title="초대하기" onClose={closeSheet} onBack={back}>
      ...
      <p className="mt-3 ml-1 text-[11.5px] leading-[1.6] text-ink3">
        코드를 아는 사람은 누구나 들어올 수 있어요. 찜찜하면 방 설정에서{' '}
