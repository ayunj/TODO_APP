...
interface GomdoriValue {
   ...
   *
   * 이걸로 지키는 것이 아니다 — 통과 가격표를 막는 것은 RLS(`is_shop_admin()`)다.
  ...
  const refreshShop = useCallback(async (): Promise<Shop> => {
    ...
    /*
      **빈 상점도 그대로 받는다.** 전에는 `안 받은 것으로 치고` 박아둔 목록을
      세웠는데, 그러면 **가격표를 비운 뒤에도 옛 목록이 상점에 뜬다** —
      서버에 없는 옷이 이름과 가격을 달고 회색 네모로 선다.
      ...
      */
      ...
      /*
        **샀다는 말을 한 줄 남긴다.** 상점에서 곰돌이 칸을 걷어낸 뒤로
        ...
        한마디 없으면 포인트만 나가고 아무 일도 안 일어난 것처럼 읽힌다.
