 ...
 *
 * **가격은 화면에 적어두는 것이다.** 진짜로 얼마를 치를지는 서버가 정한다
...
export interface Costume {
  ...
  season?: string;
  /**
   * 그림 주소. **가격표에 적혀 오는 것이 아니라 지어 넣는 것이다** —
   ...
   * (가격표 정책이 `active or is_shop_admin()`) — 그래서 보통은 늘 참이다.
   ...
   *
   * 가격표에 줄이 생긴 날이 아니다 — 새것은 숨김으로 들어와서 그림을 그리는 동안
   * 가격표에만 앉아 있다가 켜진다. 껐다 켜도 안 바뀐다.
   */
  openedAt?: string;
}
...
export interface Shop {
  ...
  items: Costume[];
  /**
   * **짓다 만 세트들** — 곰·방·소품이 아직 다 안 찬 것.
   *
   * `sets`에는 안 든다(`준비 중` 칸이 뜨면 파는 물건처럼 읽힌다).
   * 그래도 **오고 있다는 것은 말해줄 수 있어서** 따로 담는다 —
   * 상점 메인의 [예고 띠](../screens/store/Coming.tsx)가 이걸 쓴다.
   */
  coming: CostumeSet[];
  /**
 ...
 * 묶어 파는 가격이 따로 없어서 세트 자체에는 가격이 없다.
 */
...
export interface Gomdori {
  ...
  /**
   * 지금 얼마 있나 — **서버가 센 값.**
   * 담아둔 잔액이 아니라 `가입 100P + 번 것 − 산 가격의 합`이라
   * 체크를 풀면 저절로 도로 빠진다.
   */
  points: number;
}
 ...
 * 공지 — 앱을 열면 한 번 뜨는 팝업.
 *
 * **파는 것이 아니라 가격표에 안 얹었다**(표를 따로 뒀다). 가격도 분류도 그림도 없다.
...
export interface Notice {
  ...
  version: string;
  /**
   * 사진 주소. **표에 적혀 오는 것이 아니라 `id`로 지어 넣는다**
   * (`notice/<id>.png`). 표에 있는 것은 **있나 없나**뿐이다 —
   * 글만 있는 공지가 많아서, 없는 것을 부르러 가면 열 때마다 404를 먹는다.
   */
  image?: string;
}
