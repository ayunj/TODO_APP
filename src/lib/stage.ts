 ...
 *
 * **묶이는 것은 비율이지 크기가 아니다.** 내 옷장의 걸쳐보는 칸은 카드 폭의 80%로
 * 홈보다 한 뼘 작다 — 홈은 그 칸이 화면의 주인공이지만 옷장에서는 고르는 격자가
 * 주인공이라, 홈 크기 그대로 두면 격자가 통째로 접힌 아래로 밀린다.
 * 여기 값은 다 **칸에 대한 %**라 칸이 통째로 줄면 곰돌이도 같은 비율로 줄어서,
 * 작게 걸쳐봐도 홈에 그대로 선다.
 *
 * ─── 왜 이 값인가 ──────────────────────────────────────────────
 *
