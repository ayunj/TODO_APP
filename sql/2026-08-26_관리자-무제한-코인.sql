-- 관리자는 값을 안 치른다 · 기본 둘은 값표에서 뺀다
--
-- 두 가지 일을 한다.
--
--   1. **관리자에게는 잔액이 늘 넉넉하다** — 사고 입어보는 것을 끝까지 해보려고
--   2. **기본 곰돌이와 기본 룸을 값표에서 뺀다** — 앱이 들고 나간다
--
-- ─── 왜 잔액을 안 세나 ─────────────────────────────────────────
--
 ...
 *
 * 값표에 두면 파는 것들 사이에 끼어서 **지우거나 끌 수 있는 것처럼 보인다** —
 * 실제로 값표를 비울 때 같이 지워졌고, 그 순간 곰돌이는 서 있는데
 * 옷장이 비고 방을 고를 수가 없었다.
 *
 * 이 둘은 팔 것이 아니다 —
 *
 ...
 *
 * 그래서 **앱이 늘 얹는다**(`withBundled`). 값표에 줄이 있어도 앱 것으로 덮으니
 * 이미 심겨 있다면 지워도 되고 그냥 둬도 된다 — 어느 쪽이든 화면은 같다.
 *
 * 가진 것도 앱이 정한다(`FREEBIES`). `grant_free()`는 그대로 두지만
 * **이 둘은 거기 안 기댄다** — 값이 0인 물건이 나중에 생길 때를 위한 규칙이다.
 */
...
--   select my_points();
--
-- 값표에 기본 둘이 없어야 하고, 관리자로 로그인해 있으면 `999999`가 나온다.
-- 명단에 없으면 원래 잔액이 나온다 — 담아두는 것이 없어서 명단에서 빠지면
-- 그 자리에서 돌아온다.
--
-- 상점을 열면 **기본 곰돌이와 기본 룸은 그대로 있다.** 값표에 없어도 앱이 얹는다.
--
-- ─── 통에 남은 것 ───────────────────────────────────────────────
--
