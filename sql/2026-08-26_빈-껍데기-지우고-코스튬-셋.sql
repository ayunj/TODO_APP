...
-- ─── 왜 지우나 ─────────────────────────────────────────────────
--
-- 값표에 서른네 줄이 심겨 있었는데 **그림이 있는 것은 셋뿐이었다**
-- (기본 곰돌이 · 곰토끼 · 기본 룸). 나머지 서른한 줄은 이름과 값만 있는 줄이라
-- 상점 격자에 **회색 네모**로 떴다.
--
-- 그게 `아직 안 그렸어요`로 안 읽힌다. **파는 물건으로 읽힌다** — 눌러서 걸쳐보면
-- 곰돌이가 그대로고, 300P를 치르고 나서도 그대로다. 값을 치를 수 있는 자리에
...
-- ─── 코스튬 셋 ──────────────────────────────────────────────────
/*
 * **공주와 마법사는 이미 값표에 있다.** 지난번에 이름과 값만 심어두고
 ...
 *
 * 열쇠 둘을 여기 적지 않고 **`값이 0인 것`으로 고른다.** 적어두면 나중에
 ...
 *
 * 값이 0이라 `my_points()`의 뺄셈에는 아무 영향이 없다.
 */
